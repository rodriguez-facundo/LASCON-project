...
def location(n, r, rot=0):
    ...
    points = ls(0, 2*np.pi, n+1)[:-1] + rot *np.pi/180
...
simConfig.duration = 1*1e2
simConfig.dt = 0.025

simConfig.hParams = {'v_init': -65}
simConfig.verbose = 1

# choose colors for raster plot
colors = dict()
for i in range(500): colors['GC'+str(i)] = 'black'
for l, c in zip(labels[:-1], ['blue', 'red', 'blue']): colors[l] = c

simConfig.analysis['plotRaster'] = {'popColors':colors, 'markerSize':10, 'marker': 'o'}
